package com.suslovila.cybersus.client.clientProcess.processes;

import com.suslovila.cybersus.api.process.ClientProcess;
import com.suslovila.cybersus.api.process.ISerializableProcess;
import com.suslovila.cybersus.client.RenderPlayerAccelerationEffect;
import com.suslovila.cybersus.client.RenderPlayerCustom;
import com.suslovila.cybersus.client.clientProcess.processes.illusion.WrappedRenderManager;
...
public class ProcessPlayerAccelerationEffect extends ClientProcess implements ISerializableProcess {

    public int implantOwnerId;
    public double colorPercent;
    public RenderPlayerAccelerationEffect renderPlayerCustom;

    public ProcessPlayerAccelerationEffect(EntityPlayer hacker, SusVec3 position, int duration, double colorPercent) {
        super(position, duration);
        this.implantOwnerId = hacker.getEntityId();
        this.colorPercent = colorPercent;
    }

    public ProcessPlayerAccelerationEffect() {
    }
    ...
    public void render(RenderWorldLastEvent event) {
        ...
        if (clientPlayer.worldObj.getEntityByID(this.implantOwnerId) instanceof EntityPlayer) {
            EntityPlayer hacker = (EntityPlayer) clientPlayer.worldObj.getEntityByID(this.implantOwnerId);



            if(hacker == clientPlayer) {
...
//            glEnable(GL_COLOR_MATERIAL);
            ...
            SusVec3 vecFromPlayerToHacker = SusGraphicHelper.getRenderPos(hacker, event.partialTicks).subtract(SusGraphicHelper.getRenderPos(clientPlayer, event.partialTicks));
...
//            glRotated(angle, 0.0, 1.0, 0.0);
            ...
            SusGraphicHelper.translateFromPlayerTo(this.position.add(new SusVec3(0.0, hacker.height - 0.15, 0.0)), event.partialTicks);
...
//
    ...
    public String getTypeId() {
        return "player_acceleration_effect";
    }
...
//        RenderManager.instance.renderEntityWithPosYaw(entity, 0.0, 0.0, 0.0, 0.0f, 1.0f);
    ...
    public void writeTo(ByteBuf byteBuf) {
        ...
        byteBuf.writeInt(implantOwnerId);

        byteBuf.writeDouble(colorPercent);
    }

    @Override
    public void readFrom(ByteBuf byteBuf) {
        super.readFrom(byteBuf);
        implantOwnerId = byteBuf.readInt();

        colorPercent = byteBuf.readDouble();
        renderPlayerCustom = new RenderPlayerAccelerationEffect(this);
